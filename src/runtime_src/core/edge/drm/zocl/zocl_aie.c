...
zocl_aie_request_part_fd(struct drm_zocl_dev *zdev, void *data)
{
	...
	printk("__larry_zocl__: in %s: fd is %d.\n", __func__, fd);
	if (fd < 0) {
...
zocl_create_aie(struct drm_zocl_dev *zdev, struct axlf *axlf)
{
	...
	printk("__larry_zocl__: in %s find AIE_METADAT section\n", __func__);
	mutex_lock(&zdev->aie_lock);
	...
	printk("__larry_zocl__: in %s: aie_dev is %p.\n", __func__, zdev->aie->aie_dev);
...
zocl_destroy_aie(struct drm_zocl_dev *zdev)
{
	...
	if (zdev->aie->aie_dev) {
		printk("__larry_zocl__: in %s: calling release partition.\n", __func__);
