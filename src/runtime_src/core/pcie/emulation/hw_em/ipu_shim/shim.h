...
#define PRINTENDFUNC if (mLogStream.is_open()) mLogStream << __func__ << " ended " << std::endl;
 ...
 typedef struct
 {
    ...
    uint32_t reserved;          /*Reserved for the future*/
 } CuConfig;
  ...
  class HwEmShim {
    ...
    public:
      ...
      // Bitstreams
      int xclLoadXclBin(const xclBin *buffer, int iSlotID=0);
      ...
      struct sXclBinInfo
      {
        ...
        // uint32_t slot_index;
        std::vector<kds_info> m_kds_info_list;
      };

/*
      struct saie_partition_unique_metadata
      {
        bool operator == (const saie_partition_unique_metadata& lhs)
        {
          return (mslotid==lhs.m_unique_data.first) && (mcounter==lhs.m_unique_data.second);
        }
      ...
      */
      sXclBinInfo get_xclbininfo_from_map(int islotid);
      void erase_xclbininfo_from_map(int islotid);
      private:
      ...
      xrt_core::query::xclbin_slots::result_type getSlotInfo();
      xrt_core::query::kds_cu_info::result_type get_kds_cu_info();
      void convert_uuid_to_string ( const uuid_t  iUUID, std::string &oUUID);
      std::string convert_uuid_to_string(const uuid_t  iUUID);
      int get_cu_index(unsigned int );
      std::list<xclemulation::DDRBank> getMemoryManager()
      {
        return mDdrBanks;
      }
    ...
    private:
      ...
      std::mutex m_seq_memory_manager;
      //std::mutex mBitStreamMutex;
      ...
      std::shared_ptr<xclipu::cIpuManager> IpuManager;
      ...
      void* buf;
      size_t buf_size;
      mutable std::ofstream mLogStream;
      ...
      // IPU related
      std::mutex mSeq_LoadXclBin;
      std::atomic<bool> mIpurb_initialized;
      hwemu::xocl_ipurb* m_ipurb;
      //xrt_core::xclbin::aie_partition_obj aie_partition;
      struct sCUInfo_aie_partition
      {
        ...
        // add ds to store kds info of same xclbin
        sCUInfo_aie_partition() {
          mCUs =0;
        }
      };
