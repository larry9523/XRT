...
#define MEM_BANK_SHIFT_BIT 16
#define GET_SLOT_INDEX(x) (((x) >> MEM_BANK_SHIFT_BIT) & 0xFF)
...
//-----------------------------------------------IPU Specific--------------------------------------------------
  HwEmShim::sXclBinInfo HwEmShim::get_xclbininfo_from_map(int islotid)
  {
  ...
  {
    std::lock_guard<std::mutex> lk{ m_XclBinInfo_mtx };
    mxclbin_metadata.erase(islotid);
  }
  uint64_t HwEmShim::get_slotID(const uint64_t iCntxID)
  {
    std::lock_guard<std::mutex> lk{ m_XclBinInfo_mtx };
    for (const auto& x : mxclbin_metadata)
    {
       for (const auto &j : x.second.m_kds_info_list)
      {
        if (j.cuindex == iCntxID)
        {
          DEBUG_MSGS_COUT(" get slotID-  contex id is "<< iCntxID <<" found & its SlotID is "<<x.first);
          return x.first;
        }
      }
    }
  ...
  xrt_core::query::xclbin_slots::result_type HwEmShim::getSlotInfo() {
    ...
    for (const auto& slots: mxclbin_metadata) {
      ...
      //std::stringstream ss;
      //ss<<slots.first.mslotid;
      lslot_info.slot = slots.second.m_SlotIDIndex;
      lslot_info.uuid = slots.second.m_Uuid;
      lresult.push_back(lslot_info);
    }
    return lresult;
  }
  ...
  xrt_core::query::kds_cu_info::result_type HwEmShim::get_kds_cu_info()
  {
    ...
    //for (const auto &i : m_slot_to_xclbin_info_map)
    for (const auto &i : mxclbin_metadata)
    {

      xrt_core::query::kds_cu_info::data_type ldata;
      for (auto &j : i.second.m_kds_info_list)
      {
        ldata.slot_index = i.second.m_SlotIDIndex;
        ldata.index = j.cuindex;
        ldata.name = j.name;
        ldata.base_addr = j.base_addr;
        ldata.status = j.status;
        ldata.usages = j.usages;
        ldata_list.push_back(ldata);
      }
    }
    return ldata_list;
  }
  void HwEmShim::convert_uuid_to_string ( const uuid_t  iUUID, std::string &oUUID) {
    uuid_t luuid;
  ...
  std::string HwEmShim::convert_uuid_to_string(const uuid_t  iUUID) {
    std::string lstring;
    convert_uuid_to_string(iUUID, lstring);
    return lstring;
  }
  ...
  std::string HwEmShim::get_uuid_as_string(int slotid)
  {
   ...
   // DEBUG_MSGS_COUT(" m_slot_to_xclbin_info_map size is::"<<mxclbin_metadata.size());
    ...
    std::cerr<<"\n unable to get UUID for slotID::"<<slotid;
    return std::string();
  }
  ...
  int HwEmShim::ipu_create_hw_context(const uuid_t xclbinId, int slotid)
  {
    ...
    DEBUG_MSGS_COUT("ipu_create_hw_context uuid " << suuid <<"\t slotid is::"<<slotid);

    if (!xclemulation::config::getInstance()->isIpuRBMode())
      return -ENOTSUP;


    auto lhwctx = IpuManager->get_hwctx(slotid);
    if (!lhwctx) {
      std::cout << "\n There is no match to UUID so no hwctx exits, return now!!";
      return -1;
    }
    ...
    appid = slotid;
    CuConfig cfg[lhwctx->sCUinfo.mCUs];

    auto cu_idx = m_CUIndexCounter++;
    if (cu_idx == 1)
    {
      m_CUIndexCounter.reset(cu_idx);
      cu_idx = 0;
    }
    else
    {
    ...
    for (uint32_t i = cu_idx; i < lhwctx->sCUinfo.mCUs + cu_idx; ++i)
    {
      DEBUG_MSGS_COUT(" ----------------config cu _idx is  " << i << " The index is " << i - cu_idx << " size is " << lhwctx->sCUinfo.mCUs );
      cfg[i - cu_idx].cu_idx = i;
      cfg[i - cu_idx].cu_functional = lhwctx->sCUinfo.mCUFunctionalMap[i - cu_idx];
    }

    DEBUG_MSGS_COUT(" ----------------config uuid " << suuid);
    ret = m_ipurb->config(lhwctx->sCUinfo.mCUs, cfg, slotid);
    if (ret < 0)
    {
      //xrs_release_resource(xrs_hdl, counter_rid);
      // Close context if configuration fails
      m_ipurb->close_context(slotid);
      return ret;
    }
    else
    {
      ...
      sXclBinInfo lXclBin;
      lXclBin.m_Uuid = suuid;
      DEBUG_MSGS_COUT(" the UUID lXclBin.m_Uuid  is::" << lXclBin.m_Uuid);
      lXclBin.m_start_col = start_col;
      lXclBin.m_no_of_col = ncol;
      lXclBin.m_SlotIDIndex = slotid;
      lXclBin.m_rid = counter_rid;
      for (const auto& x : lhwctx->sCUinfo.mCU_kds_info_list)
      {
        kds_info lkds;
        lkds.cuindex = (m_CUIndexCounter++ - 1); // cu index
        lkds.name = x.name;                // kernel name
        lkds.base_addr = x.base_addr;      // 0 -dummy  ( IPU specific)
        lkds.status = 0;                   // 0 -dummy ( IPU specific)
        lkds.usages = 0;
        lXclBin.m_kds_info_list.push_back(lkds);
      }
  ...
  int HwEmShim::ipu_destroy_hw_context(uint32_t slotID)
  {
    DEBUG_MSGS_COUT(" funtion start");
    if (!xclemulation::config::getInstance()->isIpuRBMode())
      return -ENOTSUP;
    //DEBUG_MSGS_COUT(" cleaning resource resolver \t::" << m_slot_to_xclbin_info_map[slotID].m_rid);
   // auto status = xrs_release_resource(xrs_hdl, m_slot_to_xclbin_info_map[slotID].m_rid);
   // DEBUG_MSGS_COUT("cleaning resource resolver cleaned \t::"<< status);

    auto ret = m_ipurb->close_context(slotID);
    if (!ret)
    {
      DEBUG_MSGS_COUT(" successfully close_context on slotID::"<<slotID);
    }
    else
      DEBUG_MSGS_COUT("Failed close_context on slotID::"<<slotID);

    std::vector<uint64_t>  CUIndex_list;
    if (true)
    {
      auto lxclbininfo = get_xclbininfo_from_map(slotID);
...
/*
      ...
      // erase slotID related UUID information.
      m_slot_to_xclbin_info_map.erase(slotID);
      */
    }
  ...
  int HwEmShim::xclLoadXclBin(const xclBin* header, int iSlotID)
  {
    DEBUG_MSGS_COUT(" start.");
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
    }
    // Let's carefully run at sequentially, the reason is XclBitStreamWorker should be called only once.
   // std::lock_guard<std::mutex> lSequential{ mSeq_LoadXclBin };
    ...
    //xclemulation::bitStreamArg loadBitStreamArgs;
    ...
    // IPU Layout
    loadBitStreamArgs.m_ipuData = ipuData.get();
    loadBitStreamArgs.m_ipuDataSize = ipuDataSize;

    loadBitStreamArgs.m_top = top;
    if (false == mIpurb_initialized.load())
    {
      returnValue = xclLoadBitstreamWorker(loadBitStreamArgs);
      if (returnValue < 0)
        std::cerr << "\n xclBitStreamWorker is faield, need to exit now!\n";
    }
    else
      DEBUG_MSGS_COUT(" xclBitStreamWorker is already initiated so skipping it now!");
    ...
    {
      std::string initMsg = "INFO: [HW-EMU 01] Hardware emulation runs simulation underneath. Using a large data set will result  \
        in long simulation times.It is recommended that a small dataset is used for faster execution.The flow uses approximate models \
        for Global memoriesand interconnectand hence the performance data generated is approximate.";
          logMessage(initMsg);
    }
    ...
    DEBUG_MSGS_COUT(" SlotID "<<iSlotID<< " for UUID is\t" << top->m_header.uuid << "\t end ");
    PRINTENDFUNC;
    return 0;
  }
...
uint32_t HwEmShim::getAddressSpace (uint32_t topology)
{
  ...
  size_t HwEmShim::xclCopyBufferHost2Device(uint64_t dest, const void *src, size_t size, size_t seek, uint32_t topology)
  {
    ...
    if(!sock)
    {
      DEBUG_MSGS_COUT(" xclCopyBufferHost2Device sock object is not created yet. ");
      if(!mMemModel)
    ...
    while(processed_bytes < size){
      ...
      //std::cout<<"\n xclCopyBufferHost2Device xclCopyBufferHost2Device_RPC_CALL happening...\n";
      xclCopyBufferHost2Device_RPC_CALL(xclCopyBufferHost2Device,handle,c_dest,c_src,c_size,seek,space);
#endif
      processed_bytes += c_size;
    }
     // std::cout<<"\n xclCopyBufferHost2Device xclCopyBufferHost2Device_RPC_CALL finished\n";
  ...
  {
    //Ensure GlobalMemory Manager is used cautiously in multi-threaded approach.
    DEBUG_MSGS_COUT(" started.");
    std::lock_guard<std::mutex> lk{ m_seq_memory_manager };
    if (mLogStream.is_open()) {
    ...
    auto lflags = boFlags;
    auto slotid = GET_SLOT_INDEX(lflags);  // Get 8-bits of info only.
    //auto uuid = get_uuid_as_string(slotid);
    auto lhwctx = IpuManager->get_hwctx(slotid);
    ...
    // need check size of DDRMemorymanager
    uint64_t origSize = size;
    unsigned int paddingFactor = xclemulation::config::getInstance()->getPaddingFactor();
    uint64_t result = -1;

    if (boFlags & XCL_BO_FLAGS_HOST_ONLY) {
      DEBUG_MSGS_COUT(" inside if , Global Memory - mDDRMemoryManager creation, normal host_sptag_idx is::" << host_sptag_idx);
    ...
    else
    {
      if (ipurb && lhwctx)
      {
        DEBUG_MSGS_COUT("\n inside else ,local mDDRMemoryManager specific flags is::" << flags);
        result = lhwctx->mDDRMemoryManager[flags]->alloc(size, paddingFactor, chunks);
      }
      else
      {
        DEBUG_MSGS_COUT("\n inside else , Global mDDRMemoryManager, flags is::" << lhwctx->host_sptag_idx);
    ...
    DEBUG_MSGS_COUT(" final address is::" << finalValidAddress);
    return finalValidAddress;
  }
  ...
  HwEmShim::HwEmShim(unsigned int deviceIndex, xclDeviceInfo2 &info, std::list<xclemulation::DDRBank>& DDRBankList, bool _unified, bool _xpr,
    ...
    ,mIpurb_initialized{false}
    ,mCuIndx(0)
  {
    m_primery_key.store(0);
    simulator_started = false;
    tracecount_calls = 0;
    mReqCounter = 0;
    simulatorType = "xsim";
...
/*********************************** Utility ******************************************/
  ...
  // duplicate xocl_bo_ddr_idx function
  static unsigned xocl_bo_ddr_idx(unsigned flags, bool is_sw_emu = true)
  {
    if(is_sw_emu)
      return 0;
...
static bool check_bo_user_flags(HwEmShim* dev, unsigned flags)
{
  ...
  ddr = xocl_bo_ddr_idx(flags,false);
  if (ddr > ddr_count)
		return false;
...
uint64_t HwEmShim::xoclCreateBo(xclemulation::xocl_create_bo* info)
{
  ...
  unsigned ddr = xocl_bo_ddr_idx(info->flags,false);
...
{
  DEBUG_MSGS_COUT(" started, flags is::"<<flags );
  // How can we get slotid from here?
  if (xclemulation::config::getInstance()->isIpuRBMode()) {
    ...
    if (!(flags & XCL_BO_FLAGS_HOST_ONLY) && !(flags & XCL_BO_FLAGS_EXECBUF) && m_ipurb) {
      auto lflags = flags;
      auto slotid = GET_SLOT_INDEX(lflags);  // Get 8-bits of info only.
      if (m_ipurb->alloc_bo(size, slotid))
        return mNullBO;
    }
  }
...
{
  DEBUG_MSGS_COUT("flags-- is::" << flags);
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << ", " << std::endl;
  }
  ...
  if (xclemulation::config::getInstance()->isIpuRBMode()) {
    // Call IPU Ring Buffer to allocate a shadow buffer for SRAM
    if (!(flags & XCL_BO_FLAGS_HOST_ONLY) && !(flags & XCL_BO_FLAGS_EXECBUF) && m_ipurb) {
      DEBUG_MSGS_COUT(" xclAllocUserPtrBO inside if the flag value is::" << flags);

      //uint32_t slotid = flags & 0xFF0000; //( kSlotIDMask << 16 );  // Get 8-bits of info only.
      auto lflags = flags;
      uint32_t slotid = GET_SLOT_INDEX(lflags);
      DEBUG_MSGS_COUT(" alloc_bo started for slotid value is::" << slotid);
      if (m_ipurb->alloc_bo(size, slotid))
        return mNullBO;
      DEBUG_MSGS_COUT(" alloc_bo finished for slotid value is::" << slotid);
  ...
  xclemulation::xocl_create_bo info = { size, mNullBO, flags };
  DEBUG_MSGS_COUT(" xoclCreateBo started for slotid value is::" << GET_SLOT_INDEX(flags));
  uint64_t result = xoclCreateBo(&info);
  DEBUG_MSGS_COUT(" xoclCreateBo finished for slotid value is::" << GET_SLOT_INDEX(flags));
  //std::cout<<"\n result in xclAllocUserPtrBO is::"<<result;
  ...
  if(bo)
  {
    ...
    if (xclemulation::config::getInstance()->isIpuRBMode()) {
      ...
      if (!(bo->flags & XCL_BO_FLAGS_HOST_ONLY) && !(bo->flags & XCL_BO_FLAGS_EXECBUF) && m_ipurb) {
        lk.unlock();
        auto lflags = bo->flags;
        slotid = GET_SLOT_INDEX(lflags);
        // flags get slotid
        m_ipurb->free_bo(slotid);
        lk.lock();
      }
    }
...
int HwEmShim::get_cu_index(unsigned int cmdBO)
{
  xclemulation::drm_xocl_bo *bo = xclGetBoByHandle(cmdBO);
  auto ert_pkt = (struct ert_packet *)bo->buf;
  auto ert_start_cu = reinterpret_cast<ert_start_kernel_cmd *>(ert_pkt);
  auto mask = ert_start_cu->cu_mask;

// Need to test with multiple CU's

  uint32_t cu_idx = 0;
std::cout<<"\n mask from cmdBO is::"<<mask<<"\t in binary representation ::"<<std::bitset<sizeof(uint32_t)>(mask)<<std::endl;
 //10101000
  while (mask)
  {
    if (mask & 0x1)
  ...
  std::cout << "\n the cu index calclulated is::" << cu_idx;
  return cu_idx;
}
...
// tyecast cmdBO to ert cocommand , get cumask and retrieve cu index from it.
  xclemulation::drm_xocl_bo* bo = xclGetBoByHandle(cmdBO);
  int ret=-1;
  if(xclemulation::config::getInstance()->isNewMbscheduler())
  {
      ...
      //std::cout<<"\n cu index guessing ::"<<cu_index;
      auto slotID = get_slotID(cu_index);
      //std::cout<<"\n slot id guessing ::"<<slotID;
      // get CU index from cmdBO
      ret = m_ipurb->add_exec_buffer(bo, slotID);
      PRINTENDFUNC;
      return ret;
  } else {
    if(!mMBSch || !bo)
...
xclCloseContext(const uuid_t xclbinId, unsigned int ipIndex)
{
  ...
  DEBUG_MSGS_COUT(" ------------------------------------------xclCloseContext is called, ignoring as destroy_hw_context will take care of.");
  return 0;
...
create_hw_context(const xrt::uuid& xclbin_uuid, const xrt::hw_context::qos_type& qos, xrt::hw_context::access_mode mode)
{
  ...
  if (!lxclbin)
  {
    std::cerr << "\n xclbin is not found for UUID\n";
    return 0;
  }

  if (uuid_rid_map.find(luuid) == uuid_rid_map.end())
  {
    DEBUG_MSGS_COUT(" Calling xclloadxclbin as uuid entry is not found!");
    xclLoadXclBin(lxclbin->get_axlf(), lSlotID);
  }
  else
  {
